...
//  ex) bing.com ==> 0462696e6703636f6d00 (EOT and NULL char included)
...
const DNS_ZONE_MAP = {
  '172.217.11.174': Buffer.from('0462696e6703636f6d00', 'hex'),
  '151.101.129.140': Buffer.from('0866616365626f6f6b03636f6d00', 'hex'),
  '13.33.229.129': Buffer.from('047562657203636f6d00', 'hex'),
};
...
class DNSMessage {
  ...
  extractHeader(headerBuffer) {
    ...
    const qrOpCodeAATCRDBytes = Buffer.from('89', 'hex'); // QR, OpCode(I), AA, TC, and RD bits(1000 1001)
  ...
  buildAnswer() {
    ...
    const ipDomainTmpMap = { [qnameAscii]: Buffer.from('00000000', 'hex') };
    Object.keys(DNS_ZONE_MAP).forEach(domain => {
      if (ipDomainTmpMap[domain] !== undefined) {
        ipDomainTmpMap[domain] = DNS_ZONE_MAP[domain];
      }
    });

    // Needed because of invalid encoding for
    // the qname received from input stream
    const arrOfIPBytes = qnameAscii.split('.').map(str => +str);
    this.msgAnswer = {
      name: ipDomainTmpMap[qnameAscii],
      type: Buffer.from('0001', 'hex'),
      class: Buffer.from('0001', 'hex'),
      ttl: Buffer.from('0000003C', 'hex'),
      rdlength: Buffer.from('0004', 'hex'),
      rdata: Buffer.from(arrOfIPBytes, 'hex'),
    };
